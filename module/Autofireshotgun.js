Hooks.once("ready", () => {
  console.info("🎯 Shotgun Automatic Control Group Dialog Script Loaded (v18 - Clean Ammo Menu)");
});
...
Hooks.on("createChatMessage", async function (message) {
  ...
  // === 1. Check if the weapon fired is our Shotgun Automatic dummy weapon ===
  if (!firedWeapon.name.toLowerCase().includes("shotgun automatic control group")) {
    return; 
  }
  ...
  for (const w of allOtherWeapons) {
    ...
    const hasTheUpgrade = upgradeIds.some(id => {
      ...
      return upg && upg.name.toLowerCase().includes("shotgun automatic control group");
    });
  ...
  new Dialog({
    ...
    content: `
      ...
      <div class="form-group" style="display: flex; align-items: center; margin-bottom: 15px;">
        ...
        <select id="shotgun-ammo" style="width: 120px;">
    ...
    buttons: {
      hit: {
        ...
        callback: async (html) => {
          let mult = parseInt(html.find('#shotgun-multiplier').val());
          ...
          // Dynamically scale dice size based on amount rolled so they fit in the card
              ...
              <div class="rollcard-top">
                <div class="cpr-block chat-rollTitle-stat">
                  <div class="text-center text-padding-top text-normal text-semi">
                    Shotgun Autofire Damage (${mainWeapon.name})
                  </div>
      ...
      miss: {
        ...
        label: "Missed",
        callback: () => {
           ui.notifications.info(`Shotgun autofire attack missed. ${ammoCost} rounds were still spent from the ${mainWeapon.name}.`);
        }
      }
    },
    default: "hit"
